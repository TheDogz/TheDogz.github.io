...
function go() {
	...
	if (isNaN(dh) || isNaN(dm) || isNaN(ds) || ds == '' || dm == '' || dh == '') {
		if (isNaN(y) || isNaN(mo) || isNaN(d) || isNaN(h) || isNaN(mi) || isNaN(s)) {
			...
			console.log(endTime);
			window.location.href = 'https://thedogz.github.io/hrtcd/index.html?time=' + endTime.getTime();
		}
	} else {
		let extra = 3600000 * dh + 60000 * dm + 1000 * ds;
		let endTime = new Date().getTime() + extra;
		window.location.href = 'https://thedogz.github.io/hrtcd/index.html?time=' + endTime;
	}
}